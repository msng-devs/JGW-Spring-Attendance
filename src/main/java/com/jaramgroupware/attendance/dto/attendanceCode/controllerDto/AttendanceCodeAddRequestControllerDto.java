...
public class AttendanceCodeAddRequestControllerDto {
    ...
    @Max(value = 2592000L,message = "exp_sec -> 설정가능한 최대 유효 시간은 2592000(초) 입니다.")
    @Min(value = -1L,message = "exp_sec -> 해당 필드에 음수는 사용할 수 없습니다. 만약 영구적인 출결 코드를 발급할려면 (-1)을 입력하세요. ")
    private Long expSec;

    public AttendanceCodeAddRequestServiceDto toServiceDto(String code, Long timeTableId){
        return AttendanceCodeAddRequestServiceDto.builder()
                .expSec(expSec)
                .code(code)
